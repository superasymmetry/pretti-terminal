...
const COMMANDS = ['record', 'capture', 'render', 'config', 'install'] as const;
...
async function main(): Promise<void> {
  ...
  const command: Command = isCommand(first) ? first : 'record';
  if (isCommand(first)) process.argv.splice(2, 1);
  ...
  if (command === 'config') {
    ...
    if (!result.ok) {
      if (result.reason !== 'bad-channel') reportMissing();
      process.exitCode = 1;
    }
